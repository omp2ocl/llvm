 ...
 * are 0 to get_work_dim() \96 1. For other values of
 ...
 * values of dimindx are 0 to get_work_dim() \96 1. For
 ...
 * get_work_dim() \96 1. For other values of dimindx,
 ...
 * Valid values of dimindx are 0 to get_work_dim() \96 1.
 ...
 * number from 0 .. get_num_groups(dimindx) \96 1.
 * Valid values of dimindx are 0 to get_work_dim() \96 1.
...
/**
 * Round to integral value using the round to \96ve
 * infinity rounding mode.
 */
...
/**
 * Modulus. Returns x \96 y * trunc (x/y).
 */
...
/**
 * Returns fmin( x \96 floor (x), 0x1.fffffep-1f ).
...
/**
 * Compute sine. x must be in the range -2^16 \85 +2^16.
 */
...
/**
 * Compute tangent. x must be in the range -216 \85 +216.
 */
...
/**
 * Returns | x \96 y | without modulo overflow.
 */
 ...
 * x + (y \96 x) * a
 * a must be a value in the range 0.0 \85 1.0. If a is not
 * in the range 0.0 \85 1.0, the return values are
 * undefined.
 */
 ...
 * +0.0, or \961.0 if x < 0. Returns 0.0 if x is a NaN.
 */
 ...
 * 3) If the device is in \93denorms are flushed to zero\94
...
/**
 * Test for infinity value (+ve or \96ve) .
 */
 ...
 * CLK_GLOBAL_MEM_FENCE \96 The barrier function
 ...
 * range [0.0 \85 1.0] for image objects created with
 * image_channel_data_type set to one of the predefined
 * packed formats or CL_UNORM_INT8, or
 * CL_UNORM_INT16.
 * read_imagef returns floating-point values in the
 * range [-1.0 \85 1.0] for image objects created with
